...
if grep -q "\[torbox\]" /rclone_config/rclone.conf 2>/dev/null; then
    ...
    rclone mount torbox: /mnt/torbox --config /rclone_config/rclone.conf --vfs-cache-mode full --allow-non-empty --allow-other --dir-cache-time 5s --attr-timeout 2s --daemon
else
